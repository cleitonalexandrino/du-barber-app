  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out fill-mode-both">
      <div className="text-center space-y-2 mb-10 relative">
        <h2 className="text-3xl font-serif font-bold tracking-tight text-primary">Escolha o Artesão</h2>
        <p className="text-muted-foreground text-sm font-medium">Quem cuidará do seu visual hoje?</p>
      </div>
            ...
            >
              ...
              <CardContent className="p-8 flex items-center gap-6">
                <div className="relative">
                   ...
                   <Avatar className={`relative w-20 h-20 border-4 transition-all duration-500 ${isSelected ? 'border-accent scale-105' : 'border-white shadow-xl group-hover:scale-105'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <h3 className={`font-serif font-bold text-xl transition-colors leading-tight ${isSelected ? 'text-accent' : 'text-primary group-hover:text-accent'}`}>
                    {barber.name}
                  </h3>
                  <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1">{barber.specialty || barber.role}</p>
                  
                  {barber.rating && (
                    <div className="flex items-center gap-2 mt-2 bg-slate-50 px-3 py-1 rounded-full border border-border/50 w-fit group-hover:bg-white transition-colors">
                      <Star className="w-3.5 h-3.5 fill-accent text-accent" />
                      <span className="text-xs font-bold text-primary tabular-nums">{barber.rating.toFixed(1)}</span>
                    </div>
                  )}
                </div>
